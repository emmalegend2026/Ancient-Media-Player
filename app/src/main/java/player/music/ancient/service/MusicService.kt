...
import player.music.ancient.activities.LockScreenActivity
import player.music.ancient.appwidgets.AppWidgetBig
import player.music.ancient.appwidgets.AppWidgetCard
import player.music.ancient.appwidgets.AppWidgetCircle
import player.music.ancient.appwidgets.AppWidgetClassic
import player.music.ancient.appwidgets.AppWidgetMD3
import player.music.ancient.appwidgets.AppWidgetSmall
import player.music.ancient.appwidgets.AppWidgetText
...
import player.music.ancient.extensions.toMediaSessionQueue
import player.music.ancient.extensions.uri
import player.music.ancient.glide.BlurTransformation
import player.music.ancient.glide.AncientGlideExtension.getSongModel
...
import player.music.ancient.model.Song.Companion.emptySong
import player.music.ancient.model.smartplaylist.AbsSmartPlaylist
import player.music.ancient.providers.HistoryStore
import player.music.ancient.providers.MusicPlaybackQueueStore
import player.music.ancient.providers.SongPlayCountStore
import player.music.ancient.service.notification.PlayingNotification
...
import player.music.ancient.util.MusicUtil.toggleFavorite
import player.music.ancient.util.PackageValidator
import player.music.ancient.util.PreferenceUtil.crossFadeDuration
import player.music.ancient.util.PreferenceUtil.isAlbumArtOnLockScreen
import player.music.ancient.util.PreferenceUtil.isBluetoothSpeaker
import player.music.ancient.util.PreferenceUtil.isBlurredAlbumArt
import player.music.ancient.util.PreferenceUtil.isHeadsetPlugged
import player.music.ancient.util.PreferenceUtil.isLockScreen
import player.music.ancient.util.PreferenceUtil.isPauseOnZeroVolume
...
import player.music.ancient.util.PreferenceUtil.registerOnSharedPreferenceChangedListener
import player.music.ancient.util.PreferenceUtil.unregisterOnSharedPreferenceChangedListener
import player.music.ancient.volume.AudioVolumeObserver
import player.music.ancient.volume.OnAudioVolumeChangedListener
...
class MusicService : MediaBrowserServiceCompat(),
    ...
    val audioEffectManager: AudioEffectManager
        get() = playbackManager.audioEffectManager
    ...
    override fun onCreate() {
        ...
        syncAudioVolumeObserverRegistration()
        registerOnSharedPreferenceChangedListener(this)
        restoreState()
        sendBroadcast(Intent("$ANCIENT_MUSIC_PACKAGE_NAME.ANCIENT_MUSIC_SERVICE_CREATED"))
    ...
    override fun onDestroy() {
        ...
        releaseWakeLock()
        sendBroadcast(Intent("$ANCIENT_MUSIC_PACKAGE_NAME.ANCIENT_MUSIC_SERVICE_DESTROYED"))
    }
    ...
    private fun syncAudioVolumeObserverRegistration() {
        audioVolumeObserver?.unregister()
    ...
    override fun onSharedPreferenceChanged(
        ...
        when (key) {
            ...
            CROSS_FADE_DURATION, BATTERY_SAVER_MODE -> {
            ...
            PAUSE_ON_ZERO_VOLUME -> syncAudioVolumeObserverRegistration()
        }
    }
    ...
    override fun onPlayStateChanged() {
        if (!isPlaying && playback?.isInitialized == false && currentSong.isRadio) {
            // Radio failed to initialize or stopped unexpectedly
            handleChangeInternal(PLAY_STATE_CHANGED)
        } else {
            notifyChange(PLAY_STATE_CHANGED)
        }
    }
    ...
    fun playSongAt(position: Int) {
        ...
        val targetSong = playingQueue.getOrNull(position)
        val dispatcher = when {
            !playbackManager.isLocalPlayback -> Main
            targetSong?.isRadio == true -> Main
            else -> Default
        }
    ...
    fun sendPublicIntent(what: String) {
        val intent = Intent(what.replace(ANCIENT_MUSIC_PACKAGE_NAME, MUSIC_PACKAGE_NAME))
        ...
        intent.putExtra("scrobbling_source", ANCIENT_MUSIC_PACKAGE_NAME)
        @Suppress("Deprecation")
    ...
    fun updateMediaSessionMetaData(onCompletion: () -> Unit) {
        ...
        if (isAlbumArtOnLockScreen || VersionUtils.hasT()) {
            // val screenSize: Point = AncientUtil.getScreenSize(this)
    ...
    private fun registerBluetoothConnected() {
        ...
        if (isBluetoothSpeaker && !bluetoothConnectedRegistered) {
            registerReceiver(bluetoothReceiver, bluetoothConnectedIntentFilter)
            bluetoothConnectedRegistered = true
        } else if (!isBluetoothSpeaker && bluetoothConnectedRegistered) {
            unregisterReceiver(bluetoothReceiver)
            bluetoothConnectedRegistered = false
        }
    }
    ...
    private fun registerHeadsetEvents() {
        ...
        } else if (headsetReceiverRegistered && !isHeadsetPlugged) {
            unregisterReceiver(headsetReceiver)
            headsetReceiverRegistered = false
        }
    }
    ...
    companion object {
        ...
        const val ANCIENT_MUSIC_PACKAGE_NAME = "player.music.ancient"
        const val MUSIC_PACKAGE_NAME = "com.android.music"
        const val ACTION_TOGGLE_PAUSE = "$ANCIENT_MUSIC_PACKAGE_NAME.togglepause"
        const val ACTION_PLAY = "$ANCIENT_MUSIC_PACKAGE_NAME.play"
        const val ACTION_PLAY_PLAYLIST = "$ANCIENT_MUSIC_PACKAGE_NAME.play.playlist"
        const val ACTION_PAUSE = "$ANCIENT_MUSIC_PACKAGE_NAME.pause"
        const val ACTION_STOP = "$ANCIENT_MUSIC_PACKAGE_NAME.stop"
        const val ACTION_SKIP = "$ANCIENT_MUSIC_PACKAGE_NAME.skip"
        const val ACTION_REWIND = "$ANCIENT_MUSIC_PACKAGE_NAME.rewind"
        const val ACTION_QUIT = "$ANCIENT_MUSIC_PACKAGE_NAME.quitservice"
        const val ACTION_PENDING_QUIT = "$ANCIENT_MUSIC_PACKAGE_NAME.pendingquitservice"
        const val INTENT_EXTRA_PLAYLIST = ANCIENT_MUSIC_PACKAGE_NAME + "intentextra.playlist"
        const val INTENT_EXTRA_SHUFFLE_MODE =
            "$ANCIENT_MUSIC_PACKAGE_NAME.intentextra.shufflemode"
        const val APP_WIDGET_UPDATE = "$ANCIENT_MUSIC_PACKAGE_NAME.appwidgetupdate"
        const val EXTRA_APP_WIDGET_NAME = ANCIENT_MUSIC_PACKAGE_NAME + "app_widget_name"

        // Do not change these three strings as it will break support with other apps (e.g. last.fm
        // scrobbling)
        const val META_CHANGED = "$ANCIENT_MUSIC_PACKAGE_NAME.metachanged"
        const val QUEUE_CHANGED = "$ANCIENT_MUSIC_PACKAGE_NAME.queuechanged"
        const val PLAY_STATE_CHANGED = "$ANCIENT_MUSIC_PACKAGE_NAME.playstatechanged"
        const val FAVORITE_STATE_CHANGED = "$ANCIENT_MUSIC_PACKAGE_NAME.favoritestatechanged"
        const val REPEAT_MODE_CHANGED = "$ANCIENT_MUSIC_PACKAGE_NAME.repeatmodechanged"
        const val SHUFFLE_MODE_CHANGED = "$ANCIENT_MUSIC_PACKAGE_NAME.shufflemodechanged"
        const val MEDIA_STORE_CHANGED = "$ANCIENT_MUSIC_PACKAGE_NAME.mediastorechanged"
        const val CYCLE_REPEAT = "$ANCIENT_MUSIC_PACKAGE_NAME.cyclerepeat"
        const val TOGGLE_SHUFFLE = "$ANCIENT_MUSIC_PACKAGE_NAME.toggleshuffle"
        const val TOGGLE_FAVORITE = "$ANCIENT_MUSIC_PACKAGE_NAME.togglefavorite"
